...
def quick_action_card(icon: str, title: str, desc: str, href: str, accent: str) -> rx.Component:
    return rx.el.a(
        rx.el.div(
            ...
            class_name="h-11 w-11 rounded-xl bg-gradient-to-br from-orange-50 to-white border border-orange-100 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform",
        ),
        ...
        href=href,
        class_name="group bg-white border border-gray-200 rounded-2xl p-4 hover:border-orange-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300",
    )
    ...
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                class_name=rx.match(
                    h["verdict"],
                    ("HIGH", "h-2.5 w-2.5 rounded-full bg-red-500 ring-4 ring-red-100"),
                    ("MEDIUM", "h-2.5 w-2.5 rounded-full bg-yellow-500 ring-4 ring-yellow-100"),
                    ("SAFE", "h-2.5 w-2.5 rounded-full bg-green-500 ring-4 ring-green-100"),
                    "h-2.5 w-2.5 rounded-full bg-gray-400 ring-4 ring-gray-100",
                ),
            ),
        ...
        rx.el.div(
            rx.el.div(
                ...
                rx.el.a(rx.icon("eye", class_name="h-3.5 w-3.5 stroke-gray-500"), href="/history", class_name="h-7 w-7 rounded-md border border-gray-200 hover:border-orange-300 hover:bg-orange-50 flex items-center justify-center"),
                class_name="flex items-center justify-between gap-2",
            ),
            rx.el.p(h["input_text"], class_name="text-sm text-gray-800 mt-2 line-clamp-2"),
            rx.el.div(
                rx.el.div(
                    rx.el.div(
                ...
                rx.el.span(f"{h['risk_score']}% risk", class_name="text-[11px] font-semibold text-gray-700 ml-3"),
            ...
            class_name="flex-1 pb-5 ml-4 border-l-0",
        ),
...
def assistant_widget() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(class_name="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-2xl"),
        rx.el.div(
            rx.el.div(rx.icon("sparkles", class_name="h-5 w-5 text-white"), class_name="h-10 w-10 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center"),
            rx.el.div(
                rx.el.p("AI Safety Assistant", class_name="text-sm font-bold text-white"),
                rx.el.p("Powered by Groq", class_name="text-[11px] text-white/80"),
            ),
            class_name="flex items-center gap-3 relative",
        ),
        rx.el.p("Ask anything about scams, suspicious messages, or how to stay safe online.", class_name="text-sm text-white/90 mt-3 leading-relaxed relative"),
        rx.el.a(
            rx.icon("message-circle", class_name="h-3.5 w-3.5"),
            rx.el.span("Open Assistant"),
            rx.icon("arrow-right", class_name="h-3 w-3"),
            href="/assistant",
            class_name="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white text-[#E8471A] text-xs font-bold hover:bg-orange-50 mt-4 relative",
        ),
...
def emergency_help_card() -> rx.Component:
    return lift_card(
        ...
        rx.el.div(
            rx.el.a(
                rx.el.div(rx.el.p("Call 1930", class_name="text-sm font-bold text-white"), rx.el.p("National helpline", class_name="text-[11px] text-white/80")),
                rx.icon("phone", class_name="h-4 w-4 text-white"),
                href="tel:1930",
                class_name="flex items-center justify-between p-3 rounded-xl bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 transition-colors",
            ),
            rx.el.a(
                rx.el.div(rx.el.p("Report Online", class_name="text-sm font-bold text-gray-900"), rx.el.p("cybercrime.gov.in", class_name="text-[11px] text-gray-500")),
                rx.icon("external-link", class_name="h-4 w-4 stroke-gray-700"),
                href="https://cybercrime.gov.in",
                target="_blank",
                class_name="flex items-center justify-between p-3 rounded-xl border border-gray-200 hover:border-orange-300 hover:bg-orange-50 transition-colors",
            ),
...
def risk_breakdown_card() -> rx.Component:
    return lift_card(
        rx.el.div(
            ...
            rx.icon("bar-chart-3", class_name="h-4 w-4 stroke-[#E8471A]"),
            class_name="flex items-start justify-between mb-4",
        ),
        rx.el.div(
            rx.foreach(
                DashboardState.risk_categories,
                lambda c: rx.el.div(
                    rx.el.div(
                        rx.el.span(c["name"].to(str), class_name="text-xs font-semibold text-gray-800"),
                        rx.el.span(c["value"].to(str) + "%", class_name="text-xs font-bold text-gray-900"),
                        class_name="flex items-center justify-between mb-1.5",
                    ),
                    rx.el.div(
                        rx.el.div(class_name=f"h-2 rounded-full {c['color']}", custom_attrs={"style": {"width": c["value"].to_string() + "%"}}),
                        class_name="w-full h-2 rounded-full bg-gray-100",
                    ),
                ),
            ),
...
def achievements_card() -> rx.Component:
    return lift_card(
        rx.el.div(
            rx.el.div(
                rx.el.p("Achievements", class_name="text-[11px] font-semibold text-gray-500 uppercase tracking-wider"),
                rx.el.h3("Badges earned", class_name="text-base font-bold text-gray-900 mt-1"),
            ),
            rx.el.span("6 / 12", class_name="text-xs font-bold text-[#E8471A] bg-orange-50 border border-orange-200 px-2 py-1 rounded-full"),
            class_name="flex items-start justify-between mb-4",
        ),
        rx.el.div(
            rx.foreach(
                ...
                lambda a: rx.el.div(
                    rx.el.div(rx.icon(a["icon"], class_name="h-5 w-5 text-white"), class_name=f"h-12 w-12 rounded-xl {a['color']} flex items-center justify-center shadow-md mb-2 hover:scale-110 transition-transform"),
                    rx.el.p(a["title"], class_name="text-xs font-bold text-gray-900"),
                    rx.el.p(a["desc"], class_name="text-[10px] text-gray-500 leading-tight"),
                    class_name="flex flex-col items-center text-center p-2 rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/30 transition-all",
                ),
            ),
            class_name="grid grid-cols-3 sm:grid-cols-6 gap-2",
        ),
    )
...
def tips_carousel() -> rx.Component:
    return lift_card(
        rx.el.div(
            ...
            rx.el.div(
                rx.el.button(rx.icon("chevron-left", class_name="h-3.5 w-3.5"), on_click=DashboardState.prev_tip, class_name="h-7 w-7 rounded-md border border-gray-200 hover:border-orange-300 hover:bg-orange-50 flex items-center justify-center"),
                rx.el.button(rx.icon("chevron-right", class_name="h-3.5 w-3.5"), on_click=DashboardState.next_tip, class_name="h-7 w-7 rounded-md border border-gray-200 hover:border-orange-300 hover:bg-orange-50 flex items-center justify-center"),
        ...
        rx.el.div(
            ...
            rx.el.div(
                ...
                rx.el.p(DashboardState.selected_tip["desc"], class_name="text-xs text-gray-600 mt-1 leading-relaxed"),
            ),
        ...
        rx.el.div(
            rx.foreach(
                ...
                lambda t, i: rx.el.button(
                    ...
                    class_name=rx.cond(i == DashboardState.current_tip, "h-1.5 w-6 rounded-full bg-[#E8471A]", "h-1.5 w-1.5 rounded-full bg-gray-300 hover:bg-gray-400"),
                ),
            ),
...
def notification_center() -> rx.Component:
    # "Mark all read" button removed as requested
    return lift_card(
        rx.el.div(
                ...
                lambda n: rx.el.div(
                    ...
                    rx.el.div(
                        rx.el.div(
                            ...
                            rx.cond(n["unread"], rx.el.div(class_name="h-1.5 w-1.5 rounded-full bg-[#E8471A]"), rx.fragment()),
                            class_name="flex items-center gap-1.5",
                        ),
                    ...
                    class_name="flex items-start gap-2.5 p-2.5 rounded-lg hover:bg-gray-50 transition-colors",
                ),
            ),
...
def dashboard_content() -> rx.Component:
    return rx.el.div(
        ...
        # Row 2: Quick scan (charts removed — no gap left)
        ...
        # Row 4: Top scam categories + Badges (expanded to full width)
        rx.el.div(
            rx.el.div(risk_breakdown_card(), class_name="lg:col-span-1"),
            rx.el.div(achievements_card(), class_name="lg:col-span-2"),
            class_name="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4",
        ),

        # Row 5: Daily tip + Recent alerts (expanded to full width)
        rx.el.div(
            rx.el.div(tips_carousel(), class_name="lg:col-span-1"),
            rx.el.div(notification_center(), class_name="lg:col-span-2"),
            class_name="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4",
        ),
